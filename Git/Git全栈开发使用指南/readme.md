...
Git是一种分布式版本控制系统（Distributed Version Control Systems），由 Linux 之父 Linus 开发。

所谓分布式版本管理系统，就是在每一台机器上都有一个完整的仓库。

<img src="readme.assets/1460000038772947.png" alt="image-20201229212250725" style="zoom:86%;" />
...
<img src="readme.assets/1460000038772942.png" alt="在这里插入图片描述" style="zoom:67%;" />
...
![image-20201229220303023](readme.assets/1460000038772949.png)

#### (2) 仓库区/本地仓库 (Remote)

工作区有一个`.git`目录，其实这个不是工作区，是Git的版本库，存储了Git仓库的所有版本信息

![image-20201229220345268](readme.assets/1460000038772950.png)

![在这里插入图片描述](readme.assets/1460000038772948.png)
...
![image-20201229221144078](readme.assets/1460000038772955.png)
...
![img](readme.assets/1460000038772946.png)

### 5.2、Git分支

分支是为了将修改记录的整个流程分开存储，让分开的分支不受其它分支的影响，所以在同一个数据库里可以同时进行多个不同的修改。

![在这里插入图片描述](readme.assets/1460000038772945.png)

Git 为我们自动创建的第一个分支，也叫主分支，一般其它分支开发完成后都要合并到 master

![在这里插入图片描述](readme.assets/1460000038772958.png)

### 5.3、Git文件状态

在git中，文件主要有四种状态：

![Life Cycle](readme.assets/1460000038772956.png)
...
![image-20201229214855118](readme.assets/1460000038772943.png)
...
- `HTTPS`是凭据式的路径，使用此路径，向远程仓库推送代码的时候需要凭据（账号密码）；
    ...
    > ```
    >
    > ![image-20201230174621550](readme.assets/1460000038772952.png)
...
![image-20201229215107233](readme.assets/1460000038772953.png)

使用`git clone`命令克隆项目

![image-20201229215845069](readme.assets/1460000038772954.png)
...
![image-20201229222931344](readme.assets/1460000038772962.png)

可以通过`git branch`命令列出所有分支：

![image-20201229223057016](readme.assets/1460000038772960.png)
...
![image-20201229223325328](readme.assets/1460000038772961.png)
...
- 首先通过
    ...
    ![image-20201229223653744](readme.assets/1460000038772957.png)

- 接下来使用 `git add README.md`命令（可以使用`git add .`添加所有修改）将更改添加到暂存区

![image-20201229223945199](readme.assets/1460000038772959.png)
...
![image-20201229224319159](readme.assets/1460000038772967.png)
...
![在这里插入图片描述](readme.assets/1460000038772968.png)

可以通过`git log`命令查看提交历史：

![image-20201229224851871](readme.assets/1460000038772966.png)
...
![image-20201229231704254](readme.assets/1460000038772965.png)

在我们的远程仓库就可以看到这次提交了：

![image-20201229231807331](readme.assets/1460000038772969.png)

### 2.6、合并分支

最终我们的提交都是要合并到`master`分支的，首先切换到`master`分支，接着通过命令，`git merge develop`命令，将`develop`分支合并到`master`。

![image-20201229232122731](readme.assets/1460000038772964.png)
...
![image-20201229232415498](readme.assets/1460000038772963.png)

### 2.8、打标签

假如我们要发布一个版本，我们通常会给这次提交打一个标签 `git tag publish/0.0.1`

![image-20201229232931444](readme.assets/1460000038772973.png)
...
![img](readme.assets/1460000038772972.png)
...
<a id="my-stable-git-log-anchor">此处为锚点：显示当前分支的版本历史 `$ git log`</a>
...
![image-20201230175941903](readme.assets/1460000038772975.png)
...
![image-20201226170425899](readme.assets/1460000038772970.png)

- 点击Test，配置成功会提示Git版本信息

![image-20201226170737431](readme.assets/1460000038772971.png)
...
![image-20210101122023216](readme.assets/1460000038772974.png)

- 填入远程仓库地址，选择 `Clone`

![image-20210101122201172](readme.assets/1460000038772982.png)
...
![image-20210101122607056](readme.assets/1460000038772979.png)

可以看到 `Remote Branchs`,下面就是远程仓库的分支。点击远程分支，`Checkout As`,就可以把远程分支拉到本地了。

![image-20210101124703962](readme.assets/1460000038772976.png)

#### 1.2.3、更新代码

我们在提交代码前最好先更新下远程仓库的代码到本地仓库，这样可以减少不必要的冲突，更新update可以直接通过快捷键 `Ctrl + T `，也可以通过工具栏上按键来实现:

![image-20210101125000452](readme.assets/1460000038772983.png)

#### 1.2.4、提交/推送代码

在IDEA中开发好自己的代码之后如何提交到远程仓库呢？`右键项目`-->`Git`

![image-20210101125217192](readme.assets/1460000038772980.png)

- **1.Commit Directory**：commit代码(将stage区的暂存文件提交到当前分支的本地仓库，并清空stage区)，也可以push代码（把本地仓库的文件同步到远程仓库）。

![image-20210101125736901](readme.assets/1460000038772977.png)
...
![image-20210101130202600](readme.assets/1460000038772981.png)
...
![image-20210101130400728](readme.assets/1460000038772985.png)
...
![image-20210101122733992](readme.assets/1460000038772984.png)

#### 1.3.2、分支切换/比较/合并/重命名/删除

在Idea中这些功能的使用也常简单，点击分支，点击要操作的分支，就可以看到这些选项

![image-20210101123418491](readme.assets/1460000038772978.png)

### 1.4、查看提交历史

再`Version Controller`里`log`可以查看提交历史

![image-20210101130638650](readme.assets/1460000038772987.png)
...
![image-20210101131209085](readme.assets/1460000038772990.png)

- Idea提供了三个分栏，我们可以点击箭头，很方便地处理冲突

![image-20210101131313411](readme.assets/1460000038772986.png)

- 如果合并时没有处理，也可以在`Version Controller`中处理

![image-20210101131750519](readme.assets/1460000038772988.png)
...
![image-20210101133222496](readme.assets/1460000038772989.png)

#### 2.1.2、文件状态

当对文件进行修改后，有三种状态

![alt](readme.assets/1460000038773002.png)
...
![image-20210101134246844](readme.assets/1460000038772994.png)
...
![image-20210101134415788](readme.assets/1460000038772992.png)
...
![image-20210101135322345](readme.assets/1460000038772993.png)

OK，远程仓库就可以看到我们的提交了。

![image-20210101135531641](readme.assets/1460000038772996.png)

#### 2.1.5、拉取代码

也可以通过 vscode 左下角的下箭头看到仓库是否可以 拉取，然后点击菜单的 git pull 进行仓库更新 (对应 `git pull` 命令)

![image-20210101135743780](readme.assets/1460000038772991.png)
...
![image-20210101135941565](readme.assets/1460000038772997.png)
...
![image-20210101140524655](readme.assets/1460000038772995.png)

#### 2.2.2、GitLens基本使用

安装完成之后侧边的工具栏会多出一个 git 分支图标，点击就可以查看更详细的信息了。

![image-20210101141822489](readme.assets/1460000038773000.png)
...
![image-20210101142124359](readme.assets/1460000038772998.png)
...
![image-20210101142959836](readme.assets/1460000038773001.png)

在右上角点击`查看更改`,可以查看当前文件的更改。

![image-20210101142700430](readme.assets/1460000038773004.png)

![image-20210101143105278](readme.assets/1460000038773005.png)

#### 2.2.4、处理冲突

VScode 内置的 git 会告诉你有冲突的文件，然后通过 Gitlens 可以快速调整冲突。

![image-20210101145429996](readme.assets/1460000038773003.png)

![image-20210101145525530](readme.assets/1460000038773006.png)
